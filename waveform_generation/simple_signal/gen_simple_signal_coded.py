...
# Initialize signal
simple_sig_1 = np.zeros(int(duration * sampling_rate), dtype=np.complex128)
simple_sig_2 = np.zeros(int(duration * sampling_rate), dtype=np.complex128)

# Sum the tones
for f in frequencies:
    simple_sig_1 += gen_simple_signal(1,sampling_rate, f, duration)

for f in frequencies:
    simple_sig_2 += gen_simple_signal(-1,sampling_rate, f, duration)

simple_sig = np.append(simple_sig_1,simple_sig_2)

# Normalize to avoid clipping or excessive amplitude
normalize_ratio_1 = np.max(np.abs(simple_sig_1))
simple_sig_1 /= normalize_ratio_1

normalize_ratio_2 = np.max(np.abs(simple_sig_2))
simple_sig_2 /= normalize_ratio_2
...
ax1.plot(np.real(simple_sig),label="Real Part")  # Adjust the portion as needed
ax1.plot(np.imag(simple_sig),label="Imag Part") 
ax1.legend()
ax1.set_xlabel("Samples")
ax1.set_ylabel("Amplitude")
ax1.set_title("Real and Imag parts of OFDM signal")


ax2:Axes = axes[1]

# Plot the generated OFDM signal
freqs = np.fft.fftfreq(len(simple_sig_2),1/25000000)
ax2.plot(freqs,(np.fft.fft(simple_sig_2))/len(simple_sig_2))
ax2.set_xlabel("Frequency (Hz)")
ax2.set_ylabel("Amplitude of FFT (au)")
ax2.set_title("FFT of Time Domain Signal")

ax3:Axes = axes[2]

# ax3.plot(np.fft.fftshift(freqs),np.angle(np.fft.fft(simple_sig)/len(simple_sig))) # Adjust the portion as needed
# ax3.set_xlabel("Freq (Hz)")
# ax3.set_ylabel("Phase")
# ax3.set_title("Phase of FFT of generated OFDM signal")
