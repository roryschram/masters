function [Values_remod, Values_SurvData, DVBT2] = DemodremodFast(RefData,SurvData,index,P1foffset,OutputFile,Guard,CustomPilot,Pilot,JamPilotSetting,demodREMODwrite)
...
fprintf('Number of T2 Frames detected: %u \n',length(index)-1);
numP1 = length(index);
...
n = n.';
RefData = RefData.*exp((1i*2*pi*(-P1foffset)*n)/1024); % Data now fractional carrier freq corrected
SurvData = SurvData.*exp((1i*2*pi*(-P1foffset)*n)/1024);
...
SurvData = SurvData.*exp((1i*2*pi*(-P1ioffset)*n)/1024);

figure()
plot(20*log10(abs(fftshift(fft(RefData)))))

figure()
plot(20*log10(abs(fftshift(fft(SurvData)))))
...
%         FrametoRCF(Values_remod,SurvData(Surv_samplerange).*exp((1i*2*pi*(-1*(P2foffset+P2ioffset))*n(Surv_samplerange))/(Start.nfft)),Fc,Fs,Bw,[OutputFile sprintf('%u-%u',i1,i-1)],comment);
    ...
    Values1 = Values1.*exp((1i*2*pi*(-P2ioffset)*n)/(Start.nfft)); %Integer frequency offset correction
...
DVBT2.rawRefCorrected = Values1;
DVBT2.rawSurvCorrected = SurvData;
