...
void cRCF::allocateArrays()
{
    ...
    m_cfpReferenceData = reinterpret_cast<std::complex<float>* >(m_fpReferenceData);
    m_cfpSurveillanceData = reinterpret_cast<std::complex<float>* >(m_fpSurveillanceData);
...
std::string cRCF::getInfoString() const
{
    std::stringstream ss;
    ...
    ss << "File type =                " << m_cpFileType << std::endl;
    ss << "Time stamp at 1st sample = " << stringFromTimeStamp() << std::endl;
    ss << "Centre frenquency =        " << m_uiFc_Hz << " Hz" << std::endl;
    ss << "Sample rate =              " << m_uiFs_Hz << " Hz" << std::endl;
    ss << "Analogue bandwidth =       " << m_uiBw_Hz << " Hz" << std::endl;
    ss << "No of samples =            " << m_u64NSamples << std::endl;
    ss << "Comment string offset =    " << m_u64CommentOffset_B << " bytes" << std::endl;
    ss << "Comment string length =    " << m_uiCommentLength << " characters" << std::endl;
    ss << "Total file size =          " << m_u64FileSize_B << " bytes" << std::endl << std::endl;

    ss << "Comment string:" << std::endl;
    ss << "--------------------------------------------------" << std::endl << std::endl;
    ss <<  m_strComment << std::endl << std::endl;
    ss << "--------------------------------------------------" << std::endl << std::endl;
...
std::complex<float>* cRCF::getSurveillanceArrayComplexFloatPointer() const
{
    return m_cfpSurveillanceData;
}

std::complex<float>* cRCF::getReferenceArrayComplexFloatPointer() const
{
    return m_cfpReferenceData;
}

std::string cRCF::getFilename() const
{
    return m_strFilename;
}

std::string cRCF::stringFromTimeStamp() const
{
    //make string from timestamp in header
    return stringFromTimeStamp(m_i64TimeStamp_us);
}

std::string cRCF::stringFromTimeStamp(int64_t i64TimeStamp_us)
{
    ...
    return std::string(timeStrB, 26);
}

std::string cRCF::stringFromTimeStampAtSampleOffset(uint64_t u64SampleOffset) const
{
    return stringFromTimeStamp(getTimeStampAtSampleOffset_us(u64SampleOffset));
}

std::string cRCF::getComment() const
{
    return m_strComment;
}
...
void cRCF::setComment(std::string strComment)
{
    m_strComment = strComment;
}
void cRCF::setFilename(std::string strFilename)
{
    m_strFilename = strFilename;
}

void cRCF::remix(std::complex<float> *pcfData, unsigned uiLength, unsigned &uiIndex, double dFrequencyShift_Hz)
{
    ...
    {
        pcfData[i] *= exp(j * std::complex<float> (cdW * uiIndex++, 0));
    }
}
...
bool cRCF::readHeader(std::string strFilename, bool bReadCommentString)
{
...
bool cRCF::readHeader(std::ifstream& ifs, bool bReadCommentString)
{
    ...
    if(bReadCommentString)
    {
        ...
        m_strComment = std::string(cpComment, m_uiCommentLength - 1); //-1 to remove null terminating character in C string
...
uint64_t cRCF::readData(std::string strFilename, uint64_t u64StartSample, uint64_t u64NSamples)
{
...
uint64_t cRCF::readData(std::ifstream& ifs, uint64_t u64StartSample, uint64_t u64NSamples)
{
    ...
    if(!ifs.is_open())
    {
        std::cout << "cRCF::readData() Error ifstream object is not open to file" << std::endl;
        return 0;
    }
    ...
    {
        std::cout << "Rcf::readData() Warning requested nSamp excedes end of file. Trimming range to EOF" << std::endl;
...
uint64_t cRCF::readHeaderAndData(std::string sFilename, uint64_t u64StartSample, uint64_t u64NSamples, bool bReadCommentString)
{
    //read header
...
bool cRCF::writeHeader(std::string strFilename, bool bWriteCommentString)
{
    ...
    ofs.open(strFilename.c_str(), std::ofstream::binary | std::ofstream::trunc);
...
bool cRCF::writeHeader(std::ofstream& ofs, bool bWriteCommentString)
{
...
std::ifstream& operator>>(std::ifstream& ifs, cRCF& rhs)
{
    ...
    if(!ifs.is_open())
    {
        std::cout << "cRCF::operator>>() Error: ifstream file object is not open to file." << std::endl;
        return ifs;
    }
    ...
    rhs.m_strComment = std::string(cpComment, rhs.m_uiCommentLength - 1); //-1 to remove null terminating character in C string
...
std::ofstream& operator<<(std::ofstream& ofs, cRCF& rhs)
{
    ...
    if(!ofs.is_open())
    {
        std::cout << "cRCF::operator>>() Error: ofsteam object is not open to file." << std::endl;
        return ofs;
    }
...
#endif
{
    ...
    rhs.m_strComment = std::string(cpCommentString, rhs.m_uiCommentLength - 1); //-1 to remove null terminating character
...
#endif
{
    ...
    while(uiBytesLeft)
    {
        ...
        if(iBytesWritten == -1)
        {
            std::cout << "Ard::operator<<(): Error: can't write to socket client has likely closed the connection" << std::endl;
            return;
        }
    ...
    while(uiBytesLeft)
    {
        ...
        if(iBytesWritten == -1)
        {
            std::cout << "Ard::operator<<(): Error: can't write from socket client has likely closed the connection" << std::endl;
            return;
        }
...
#endif
{
    ...
    while(uiBytesLeft)
    {
        ...
        if(iBytesRead == -1)
        {
            std::cout << "Ard::operator<<(): Error: can't read from socket client has likely closed the connection" << std::endl;
            return;
        }
    ...
    m_strFilename = std::string(cpFilename, uiFilenameLength - 1); //-1 to remove null terminating character
    delete [] cpFilename;
}
...
char* cRCF::serialiseHeader()
{
    ...
    {
        //file type invalid
        std::cout << "Rcf::operator>>() Warning received file is not of known RCF type: Got '" << m_cpFileType << "' expected 'rcf'."<< std::endl;
    }
