...
classdef cRCF < matlab.mixin.Copyable
    properties %Should be private but can't because it breaks mex file access.
        ...
        m_strComment = '';
        m_strFilename = '';
        m_fvSurveillanceData = complex(zeros(0, 0, 'double')); %data is stored in double precision here
        m_fvReferenceData = complex(zeros(0, 0, 'double'));
    ...
    methods
        ...
        function oRCF = readSamplesFromFile(oRCF, StartSample, NSamples)
            ...
            oRCF.m_fvReferenceData = complex(zeros(NSamples, 1, 'double'));
            oRCF.m_fvSurveillanceData = complex(zeros(NSamples, 1, 'double'));
            ...
            %Read NSamples
            for ii = 1:NSamples
                oRCF.m_fvReferenceData(ii) = fread(f, 1, 'float64') + 1i * fread(f, 1 ,'float64');
                oRCF.m_fvSurveillanceData(ii) = fread(f, 1, 'float64') + 1i * fread(f, 1, 'float64');
            end
        ...
        function writeToFile(oRCF, strFilename)
            ...
            for ii = 1:oRCF.m_NSamples
                ...
                fwrite(outFile, real(oRCF.m_fvReferenceData(ii)), 'float64');
                fwrite(outFile, imag(oRCF.m_fvReferenceData(ii)), 'float64');
                fwrite(outFile, real(oRCF.m_fvSurveillanceData(ii)), 'float64');
                fwrite(outFile, imag(oRCF.m_fvSurveillanceData(ii)), 'float64');
            end
