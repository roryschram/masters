function [SurvData] = CGLS_Cancellation_RefSurv(RefData, SurvData, fs, proc)
...
%The number of proc.samples per segment
segmentSize_nSamp = floor(proc.samples /  proc.nSegments);

%The number of range and Doppler bins over which cancellation will be
%applied
nRangeBins = ceil((proc.cancellationMaxRange_m - proc.TxToRefRxDistance_m)/(3e8/fs));
nDopplerBinds = ceil(proc.cancellationMaxDoppler_Hz/(fs/segmentSize_nSamp)) * 2 + 1;
