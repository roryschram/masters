function [OutputAlpha,Survdata] = ProcLat(m_fvReferenceData,m_fvSurveillanceData, cancellationMaxRange_m, cancellationMaxDoppler_Hz, txToRefRxDistance_m, nSegments, nIterations, initialAlpha)
...
m_Reference_data = m_fvReferenceData;
m_Surveillance_data = m_fvSurveillanceData;
...
CPI = length(m_fvReferenceData)/(FS);
...
for segmentNo = 0:Segments - 1
    ...
    for i=1:nRangeBins
        ZeroDopplerA(i:segmentSize_nSamp, i)=m_Reference_data((segStartSampleNo:segStopSampleNo - (i - 1)));
    end
    ...
    [m, n] = size(A);
    b    = m_Surveillance_data(segStartSampleNo:segStopSampleNo);
    x    = alpha;
    ...
    oOutputRCF_Lat.m_fvSurveillanceData(segStartSampleNo:segStopSampleNo) = ...
        m_Surveillance_data(segStartSampleNo:segStopSampleNo) - (A * alpha);
...
Survdata = oOutputRCF_Lat.m_fvSurveillanceData;
clear sampleTimes_s
OutputAlpha = alpha;
end
